...
const MeetingHistory = () => {
    ...
    const handleRetryTranscription = async (meeting) => {
        try {
            ...
            if (result.success) {
                alert('✅ Transcription retry initiated! Processing will start shortly.')
                // Update meeting status to processing
                setMeetings(meetings.map(m =>
                    m.id === meeting.id ? { ...m, status: 'processing' } : m
                ))
            } else {
                alert(`⚠️ Retry failed: ${result.error}. Please try again later.`)
            }
        } catch (error) {
            console.error('Error retrying transcription:', error)
            alert('❌ Error retrying transcription. Please try again.')
        } finally {
            setRetryingId(null)
        }
    }
                            ...
                            {selectionMode && (
                                <>
                                    <button
                                        ...
                                        className={`px-4 py-2 rounded-lg transition font-medium flex items-center gap-2 ${selectedIds.length === 0 ? 'bg-gray-300 text-gray-600 cursor-not-allowed' : 'bg-red-500 text-white hover:bg-red-700'}`}
